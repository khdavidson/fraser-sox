...
# changes are committed to khdavidson/fraser-sox/ repo
...
# set working directory where Excel data entry file is
...
counts <- counts %>%
  ...
  filter(count_number != "NA") %>%                                                           # remove rows where count_number is an NA 
  mutate(sox_us_net = sox_us-sox_ds) %>%                                                     # create new column "sox_us_net" to calculate net upstream movement
  mutate(date_hr = paste(paste(gsub("-", "-", date)), count_hr_24, sep="-"))                 # create new column "date_hr" to create unique date-count hour variable       
...
enviro <- enviro %>%                                                                        # rename columns to be more R friendly
  ...
  filter(date != "", date != "**sonar pulled 0820 Oct 2, 2018") %>%                        # remove cases where date is blank, and the last row that had an anecdotal comment 
  mutate(date = lubridate::ymd(date)) %>%                                                  # reformat date to be date format
  mutate_at(vars(c(5, 12)), funs(as.numeric))                                              # reformat some variables to be numeric
