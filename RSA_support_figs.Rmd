...
daily.dat.raw <- read_excel("SONAR_daily_counts.xlsx", sheet="all_stocks")
...
esc.db <- esc.db %>% 
  mutate(start_yday = lubridate::yday(start_date),
         ...
         group = ifelse(year==lubridate::year(Sys.Date()), lubridate::year(Sys.Date()),  # generalized to current system date year (affected by time zones though!)
...
![](C:/Users/davidsonka/Documents/ANALYSIS/figures/RSA_fig4_legend.PNG)
...
<font size="2">*Figure 4. Peak of spawn ranges for forecast stocks over the entire time series available in the sockeye escapement database up to 2020. Bar sizes are scaled by relative escapement size and coloured by data quality.*</font>
