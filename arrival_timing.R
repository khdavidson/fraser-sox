...
# reproducible: extract 10% dates for all systems and years in the spreadsheet
dates_10p <- data %>% 
  group_by(stock, year) %>% 
...
arrival_10p_2019_20 <- dates_10p %>% 
  ...
  filter(stock %in% c("Cultus", "Scotch Creek", "Birkenhead", "(Horsefly River)", "(Mitchell River)", "Quesnel", "Chilko", "Nadina", 
    "Upper Chilliwack")) %>%
  select(stock, year, date, yday) %>%
  mutate(date = as.Date(date)) %>%
  mutate(p10_flag = "10% date") %>%
  print()

# join for plotting
forplot <- left_join(data, arrival_10p_2019_20, by=c("stock", "year", "date", "yday"))

# export the 10% dates 
write.csv(arrival_2020, "arrival_10p_2020.csv")
